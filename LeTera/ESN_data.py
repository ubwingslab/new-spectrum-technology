...
# #You should have received a copy of the GNU General Public License# along with this program.  If not, see <http://www.gnu.org/licenses/>.import flytera_cfg
...
def data_for_esn():
   ...
   # exit()
    ...
    var_tr_xyz, var_rx_xyz, var_tr_rpy, var_rx_rpy = variance_calc(distance_tr_xyz_list, distance_rx_xyz_list, distance_tr_rpy_list, distance_rx_rpy_list)
    mean_tr_xyz, mean_rx_xyz, mean_tr_rpy, mean_rx_rpy = mean_calc(distance_tr_xyz_list, distance_rx_xyz_list, distance_tr_rpy_list, distance_rx_rpy_list)
    
    angles = ESN_data_format.esn_data_prep(distance_data, mean_tr_xyz, mean_rx_xyz, mean_tr_rpy, mean_rx_rpy, var_tr_xyz, var_rx_xyz, var_tr_rpy, var_rx_rpy, optimal_angle) 
...
def distance():
    ...
    data_tr_xyz = np.asarray(flytera_cfg.data_tr_xyz[0:flytera_cfg.sim_tick*2+1])
    data_rx_xyz = np.asarray(flytera_cfg.data_rx_xyz[0:flytera_cfg.sim_tick*2+1])
    data_tr_rpy = np.asarray(flytera_cfg.data_tr_rpy[0:flytera_cfg.sim_tick*2+1])
    data_rx_rpy = np.asarray(flytera_cfg.data_rx_rpy[0:flytera_cfg.sim_tick*2+1])
    ...
    for i in range(flytera_cfg.sim_tick):
        distance_tr_xyz = distance_calculation(data_tr_xyz[i*2+0], data_tr_xyz[i*2+1])
        distance_rx_xyz = distance_calculation(data_rx_xyz[i*2+0], data_rx_xyz[i*2+1])
        distance_tr_rpy = distance_calculation(data_tr_rpy[i*2+0], data_tr_rpy[i*2+1])
        distance_rx_rpy = distance_calculation(data_rx_rpy[i*2+0], data_rx_rpy[i*2+1])
...
def plot_angle_capacity(data):
    ...
    matplotlib.pyplot.xlabel('Beam Steering Angle (degree)') 
...
def plots(data1, data2):
    ...
    #ang_5 = ang_5/5
    #print(ang_1)
    ang_5 = data1[4][interval:]
    #print(ang_5)
    ang_6 = data1[5][interval:]
    #print(ang_10)
    ang_20 = data1[19][interval:]
    #print(ang_20)
    ...
    print(statistics.mean(y_axis))
    print(statistics.mean(ang_5))
    print(statistics.mean(ang_20))
    
    matplotlib.pyplot.xlim(0,100)
    matplotlib.pyplot.ylim(0,4000)
    matplotlib.pyplot.plot(range(len(y_axis)), y_axis, c='k', linestyle = '-', marker = '.', label='LeTera')
    matplotlib.pyplot.plot(range(len(y_axis)), ang_5, c='b', linestyle = '--',linewidth=2.0, label='Directivity Angle = 5\xB0')
    #matplotlib.pyplot.plot(range(len(y_axis)), ang_4, c='r', linestyle = '-', marker = 'o', label='Beam Steering Angle = 4\xB0')
    #matplotlib.pyplot.plot(range(len(y_axis)), ang_6, c='b', linestyle = '-', marker = 'o', label='Beam Steering Angle = 6\xB0')
    matplotlib.pyplot.plot(range(len(y_axis)), ang_20, c='r', linestyle = ':',linewidth=5.0, label='Directivity Angle = 20\xB0')
    #matplotlib.pyplot.xlim(0,100)
...
def plot_ang_v_cap_diff_mob():
    ...
    matplotlib.pyplot.plot(range(0,75), beam1, c='b', linestyle = '-', marker = 'o',  label='Tx and Rx with windy mobility')
    matplotlib.pyplot.plot(range(0,75), beam2, c='k', linestyle = '-', marker = 'v',   label='Tx and Rx with non-windy mobility')
    #matplotlib.pyplot.plot(range(0,75), beam3, c='g', linestyle = '--', marker = '<', label='Tr with Non Windy and Rx with Windy Mobility')
    
    matplotlib.pyplot.xlim(0,75)
    matplotlib.pyplot.xlabel('Directivity Angle (degree)') 
    matplotlib.pyplot.ylabel('THz Link Capacity (Mbps)') 
    matplotlib.pyplot.legend()
    matplotlib.pyplot.grid(True)
