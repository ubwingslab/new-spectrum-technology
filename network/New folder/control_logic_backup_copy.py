...
def ctl_operation(env, nt):
    ...
    while True:
        ...
        for bs in nt.list_lte_bs_mobile:
            ...
            #print(bs_obj.served_user)
            #print('dpn',data_prediction_new)
            #print(bs_obj.get_coord())
            for ue in bs_obj.served_user:
                ue_obj = nt.get_netelmt(ue)
                ue_obj.blk_detection()
                ue_obj.set_noise()
                ue_obj.sinr_calc()
                sinr_list.append(ue_obj.sinr)
                #print(ue_obj.name,ue_obj.sinr)
            a = round(sum(sinr_list),4)
            #sinr_column = np.hstack((sinr_column,a))
            #print(a)
            if loop == 0:
                sinr_column = np.hstack((sinr_column,a))
            else:
                sinr_column = np.vstack((sinr_column,a))
            loop+=1
        ...
        for bs in range(netcfg.num_lte_bs_mobile):
            ...
            for nds in range(np.size(needed_data,0)):
                ...
                #print(nds,'-',data_prediction_new)
                new_coord['x'] = data_prediction_new[bs*3+1]
                new_coord['y'] = data_prediction_new[bs*3+2]
                new_coord['z'] = data_prediction_new[bs*3+3]
                #print(new_coord)
                bs_obj.set_coord(new_coord)   
                #print('b',bs_obj.coord_x,'...',bs_obj.coord_y,'...',bs_obj.coord_z)
                ...
                for ue in bs_obj.served_user:
                    ...
                    sinr_list.append(ue_obj.sinr)
                    #print(nds,'...',ue_obj.sinr)
                    with open("welcome5.txt",'a+') as file: # Use file to refer to the file object
                        data = str(ue_obj.coord_x)+','+str(ue_obj.coord_y)+','+str(ue_obj.coord_z)+','+str(ue_obj.sinr)+'\n'
                        file.writelines(data)
                #print(sinr_list)
                #print(sum(sinr_list))
                #print(data_prediction_new,round(sum(sinr_list),4))
                #print(bs_obj.name,bs_obj.coord_x,bs_obj.coord_y,bs_obj.coord_z,bs_obj.served_user)
            #print('\n')
            #exit(0)
            with open("welcome3.txt",'a+') as file: # Use file to refer to the file object
                file.writelines('\n\n\n\n\n\n\n\n')
            #print('after before',data_prediction_new)
            data_prediction_new[bs*3+1: bs*3+4] = data_before_change[bs]
            #print('after change',data_prediction_new,'\n')
