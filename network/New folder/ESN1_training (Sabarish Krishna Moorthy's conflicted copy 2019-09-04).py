...
def append_training_data(number_bs, train_data_coord, train_data_rate):
    ...
    #exit(0)
    coord_mbs =training_data_bs[1+ (3*netcfg.num_lte_bs): netcfg.num_lte_bs_mobile*netcfg.num_lte_bs_mobile*netcfg.num_lte_bs]
    #print(coord_mbs)
...
def esn_training(index, data, ntwk_obj):
    ...
    #data = np.hstack((data,data_rate))
    #data = np.hstack((data,pred_test))
    #print('data + pred test data',data)
    #data_coord = data[:,0:3*netcfg.num_lte_bs_mobile + 1]
    #data_utility = data[:,3*netcfg.num_lte_bs_mobile + 2:3*netcfg.num_lte_bs_mobile + 3]
    #data_esn2 = np.hstack((data_coord,data_utility))
    #print('data esn2',data_esn2)
